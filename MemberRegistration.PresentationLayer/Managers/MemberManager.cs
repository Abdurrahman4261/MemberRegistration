...
namespace MemberRegistration.PresentationLayer.Managers
{
    public class MemberManager
    {
        ...
        public List<List<int>> NumberOfAnswers(List<List<int>> numberOfAnswers)
        {
            var answers = new List<List<string>>();
            answers = FiilList(answers);
            int index = 0;

            numberOfAnswers.Add(new List<int>());
            foreach (var item in answers[index])
            {
                ...
                var list1 = _memberContext.Answers.FromSql($"select * from answers where LOWER (answer1)={item}")
                    ...
                    .ToList(); 
                numberOfAnswers[index].Add(list1.Count);
            }
            numberOfAnswers.Add(new List<int>());
            index++;
            foreach (var item in answers[index])
            ...
            foreach (var item in answers[index])
            {
                var list1 = _memberContext.Answers.FromSql($"select * from answers where LOWER (answer6)={item}")
                    .Select(a => new
                    {
                        a.Answer2
                    }).ToList();
            ...
            foreach (var item in answers[index])
            {
                var list1 = _memberContext.Answers.FromSql($"select * from answers where LOWER (answer7)={item}")
                    .Select(a => new
                    {
                        a.Answer2
                    }).ToList();
